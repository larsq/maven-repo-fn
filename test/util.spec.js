...
describe("util", () => {
  describe("paths", () => {
    it("should split into subpaths", () => {
      expect(paths("foo/bar/woz")).to.eql(["foo", "foo/bar", "foo/bar/woz"]);
    });
  });
  ...
  describe("replace", () => {
    ...
    it("should append target", () => {
      expect(replace("/path/to/asset", "/", "maven-repo")).to.equal(
        "maven-repo/path/to/asset"
      );
    });
